    ...
    <div className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          {lastRotationAt && (
            <div className="flex items-center gap-2 p-3 bg-gray-50 border border-gray-200 rounded-md">
              ...
              <div className="flex-1">
                <Text className="font-medium text-gray-700">{t('Last_Rotation')}</Text>
          ...
          {(keyRotationAt || nextRotationAt) && (
            <div className="flex items-center gap-2 p-3 bg-gray-50 border border-gray-200 rounded-md">
              <ClockIcon className="w-4 h-4 text-gray-500" />
              <div className="flex-1">
                <Text className="font-medium text-gray-700">{t('Next_Scheduled_Rotation')}</Text>
          ...
          {autoRotate && !lastRotationAt && !keyRotationAt && !nextRotationAt && (
            <div className="flex items-center gap-2 p-3 bg-gray-50 border border-gray-100 rounded-md">
              <ClockIcon className="w-4 h-4 text-gray-500" />
              <Text className="text-gray-600">{t('No_rotation_history_available')}</Text>
            </div>
          )}
        </div>
      )}
      ...
      {!autoRotate && !lastRotationAt && !keyRotationAt && !nextRotationAt && (
        <div className="flex items-center gap-2 p-3 bg-gray-50 border border-gray-100 rounded-md">
          ...
          <Text className="text-gray-600">{t('Autorotation_is_not_enabled_for_this_key')}</Text>
        </div>
      )}
    </div>
  );
    ...
    return (
      <div className={`bg-white border border-gray-200 rounded-lg p-6 ${className}`}>
        <div className="flex items-center gap-2 mb-6">
          <div>
            <Text className="font-semibold text-gray-900">{t('AutoRotation')}</Text>
            <Text className="text-xs text-gray-500">{t('Automatic_key_rotation_settings_and_stat')}</Text>
          </div>
        </div>
        {content}
      </div>
