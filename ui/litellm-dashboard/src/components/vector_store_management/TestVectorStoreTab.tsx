...
const TestVectorStoreTab: React.FC<TestVectorStoreTabProps> = ({ accessToken, vectorStores }) => {
  ...
  if (!accessToken) {
    return (
      <Card>
        <Text type="secondary">{t('Access_token_is_required_to_test_vector')}</Text>
      </Card>
    );
  }
    ...
    return (
      <Card>
        <div className="text-center py-8">
          <Text type="secondary">{t('No_vector_stores_available_Create_one_fi')}</Text>
        </div>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <Card>
        <div className="space-y-4">
          <div>
            <Title level={5}>{t('Select_Vector_Store')}</Title>
            <Text type="secondary">{t('Choose_a_vector_store_to_test_search_que')}</Text>
          </div>
