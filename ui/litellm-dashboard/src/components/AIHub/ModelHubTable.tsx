    ...
    <div className="w-full mx-4 h-[75vh]">
      {publicPage == false ? (
        <div className="w-full m-2 mt-2 p-8">
          ...
          <div className="flex justify-between items-center mb-6">
            <div className="flex flex-col items-start">
              <Title className="text-center">{t('AI_Hub')}</Title>
          ...
          <TabGroup>
            <TabList className="mb-4">
              ...
              <Tab>{t('Agent_Hub')}</Tab>
              <Tab>{t('MCP_Hub')}</Tab>
              <Tab>{t('Claude_Code_Plugin_Marketplace')}</Tab>
            </TabList>
            ...
            <TabPanels>
                ...
                <Card>
                  ...
                  {publicPage == false && isAdminRole(userRole || "") && (
                    <div className="flex justify-end mb-4">
                      <Button onClick={() => handleMakePublicPage()}>{t('Select_Models_to_Make_Public')}</Button>
                    </div>
                  )}
              ...
              <TabPanel>
                <Card>
                  ...
                  {publicPage == false && isAdminRole(userRole || "") && (
                    <div className="flex justify-end mb-4">
                      <Button onClick={() => handleMakeAgentPublicPage()}>{t('Select_Agents_to_Make_Public')}</Button>
                    </div>
                  )}
              ...
              <TabPanel>
                <Card>
                  ...
                  {publicPage == false && isAdminRole(userRole || "") && (
                    <div className="flex justify-end mb-4">
                      <Button onClick={() => handleMakeMcpPublicPage()}>{t('Select_MCP_Servers_to_Make_Public')}</Button>
                    </div>
                  )}
        ...
        <Card className="mx-auto max-w-xl mt-10">
          <Text className="text-xl text-center mb-2 text-black">{t('Public_Model_Hub_not_enabled')}</Text>
          <p className="text-base text-center text-slate-800">{t('Ask_your_proxy_admin_to_enable_this_on_t')}</p>
        </Card>
      )}
      ...
      >
        {selectedModel && (
          <div className="space-y-6">
            {/* Model Overview */}
            <div>
              <Text className="text-lg font-semibold mb-4">{t('Model_Overview')}</Text>
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  <Text className="font-medium">{t('Model_Group')}</Text>
            ...
            <div>
              <Text className="text-lg font-semibold mb-4">{t('Token_Cost_Information')}</Text>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Text className="font-medium">{t('Max_Input_Tokens')}</Text>
                  <Text>{selectedModel.max_input_tokens?.toLocaleString() || "Not specified"}</Text>
                </div>
                <div>
                  <Text className="font-medium">{t('Max_Output_Tokens')}</Text>
              ...
              <div className="flex flex-wrap gap-2">
                {(() => {
                  ...
                  if (capabilities.length === 0) {
                    return <Text className="text-gray-500">{t('No_special_capabilities_listed')}</Text>;
                  }
            ...
            {(selectedModel.tpm || selectedModel.rpm) && (
              <div>
                <Text className="text-lg font-semibold mb-4">{t('Rate_Limits')}</Text>
                <div className="grid grid-cols-2 gap-4">
                  {selectedModel.tpm && (
                    <div>
                      <Text className="font-medium">{t('Tokens_per_Minute')}</Text>
                      <Text>{selectedModel.tpm.toLocaleString()}</Text>
                    </div>
                  )}
                  {selectedModel.rpm && (
                    <div>
                      <Text className="font-medium">{t('Requests_per_Minute')}</Text>
            ...
            {selectedModel.supported_openai_params && (
              <div>
                <Text className="text-lg font-semibold mb-4">{t('Supported_OpenAI_Parameters')}</Text>
                <div className="flex flex-wrap gap-2">
            ...
            {/* Usage Example */}
            <div>
              <Text className="text-lg font-semibold mb-4">{t('Usage_Example')}</Text>
      ...
      >
        {selectedAgent && (
          <div className="space-y-6">
            {/* Agent Overview */}
            <div>
              <Text className="text-lg font-semibold mb-4">{t('Agent_Overview')}</Text>
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                ...
                <div>
                  <Text className="font-medium">{t('Protocol_Version')}</Text>
            ...
            {/* Input/Output Modes */}
            <div>
              <Text className="text-lg font-semibold mb-4">{t('InputOutput_Modes')}</Text>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Text className="font-medium">{t('Input_Modes')}</Text>
                  <div className="flex flex-wrap gap-1 mt-1">
                    {selectedAgent.defaultInputModes?.map((mode) => (
                      <Badge key={mode} color="blue">
                        {mode}
                      </Badge>
                    )) || <Text>{t('Not_specified')}</Text>}
                  </div>
                </div>
                <div>
                  <Text className="font-medium">{t('Output_Modes')}</Text>
                  <div className="flex flex-wrap gap-1 mt-1">
            ...
            {selectedAgent.skills && selectedAgent.skills.length > 0 && (
              <div>
                <Text className="text-lg font-semibold mb-4">{t('Skills_1')}</Text>
            ...
            {selectedAgent.supportsAuthenticatedExtendedCard && (
              <div>
                <Text className="text-lg font-semibold mb-4">{t('Additional_Features')}</Text>
                <Badge color="green">{t('Supports_Authenticated_Extended_Card')}</Badge>
              </div>
            )}
          </div>
        )}
      ...
      >
        {selectedMcpServer && (
          <div className="space-y-6">
            {/* Server Overview */}
            <div>
              <Text className="text-lg font-semibold mb-4">{t('Server_Overview')}</Text>
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  <Text className="font-medium">{t('Server_Name')}</Text>
                  <Text>{selectedMcpServer.server_name}</Text>
                </div>
                <div>
                  <Text className="font-medium">{t('Server_ID')}</Text>
                  <div className="flex items-center space-x-2">
                ...
                <div>
                  <Text className="font-medium">{t('Auth_Type')}</Text>
                  <Badge color={selectedMcpServer.auth_type === "none" ? "gray" : "green"}>
                    {selectedMcpServer.auth_type}
                  </Badge>
                </div>
                <div>
                  <Text className="font-medium">{t('Status_2')}</Text>
                  <Badge
                    color={
            ...
            {/* Connection Details */}
            <div>
              <Text className="text-lg font-semibold mb-4">{t('Connection_Details')}</Text>
            ...
            {selectedMcpServer.allowed_tools && selectedMcpServer.allowed_tools.length > 0 && (
              <div>
                <Text className="text-lg font-semibold mb-4">{t('Allowed_Tools')}</Text>
                <div className="flex flex-wrap gap-2">
            ...
            {selectedMcpServer.mcp_access_groups && selectedMcpServer.mcp_access_groups.length > 0 && (
              <div>
                <Text className="text-lg font-semibold mb-4">{t('Access_Groups')}</Text>
                <div className="flex flex-wrap gap-2">
            ...
            <div>
              ...
              {selectedMcpServer.health_check_error && (
                <div className="mt-2 p-2 bg-red-50 rounded">
                  <Text className="font-medium text-red-700">{t('Health_Check_Error')}</Text>
