...
const PassThroughInfoView: React.FC<PassThroughInfoProps> = ({
  ...
  if (!endpointData) {
    return <div className="p-4">{t('Pass_through_endpoint_not_found')}</div>;
  }
              ...
              <Card>
                ...
                <div className="mt-2 space-y-2">
                  ...
                  {endpointData.methods && endpointData.methods.length > 0 && (
                    <div>
                      <Text className="text-xs text-gray-500">{t('HTTP_Methods')}</Text>
                  ...
                  {(!endpointData.methods || endpointData.methods.length === 0) && (
                    <div>
                      <Text className="text-xs text-gray-500">{t('All_HTTP_methods_supported')}</Text>
                    </div>
                  )}
                ...
                <div className="mt-4 space-y-2">
                  {Object.entries(endpointData.guardrails).map(([name, settings]) => (
                    <div key={name} className="p-3 bg-gray-50 rounded">
                      ...
                      {!settings && <div className="text-xs text-gray-600 mt-1">{t('Uses_entire_payload')}</div>}
                    </div>
                  ))}
                </div>
              </Card>
            ...
            <TabPanel>
              <Card>
                <div className="flex justify-between items-center mb-4">
                  <Title>{t('Pass_Through_Endpoint_Settings')}</Title>
                ...
                {isEditing ? (
                  ...
                  >
                    ...
                    <div className="flex justify-end gap-2 mt-6">
                      ...
                      <TremorButton>{t('Save_Changes')}</TremorButton>
                    </div>
                  </Form>
                ) : (
                  <div className="space-y-4">
                    ...
                    <div>
                      <Text className="font-medium">{t('Target_URL')}</Text>
                      <div>{endpointData.target}</div>
                    </div>
                    <div>
                      <Text className="font-medium">{t('Include_Subpath')}</Text>
                      <Badge color={endpointData.include_subpath ? "green" : "gray"}>
                    ...
                    {endpointData.cost_per_request !== undefined && (
                      <div>
                        <Text className="font-medium">{t('Cost_per_Request')}</Text>
                        <div>${endpointData.cost_per_request}</div>
                      </div>
                    )}
                    <div>
                      <Text className="font-medium">{t('Authentication_Required')}</Text>
                    ...
                    <div>
                      ...
                      {endpointData.headers && Object.keys(endpointData.headers).length > 0 ? (
                        ...
                        <div className="text-gray-500">{t('No_headers_configured')}</div>
                      )}
                    </div>
                  </div>
                )}
