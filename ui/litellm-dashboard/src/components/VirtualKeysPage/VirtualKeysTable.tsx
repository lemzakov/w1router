...
const [selectedKey, setSelectedKey] = useState<KeyResponse | null>(null);
  ...
  const columns: ColumnDef<KeyResponse>[] = useMemo(() => [
    ...
    },
    {
      ...
      header: () => (
        <span className="flex items-center gap-1">
          {t('Polzovatel')}
          <Popover
            content={t('Otobrazhaet_pervoe_dostupnoe_znachenie_p')}
            trigger="hover"
          >
      ...
      cell: ({ row }) => {
        ...
        const popoverContent = (
          <div className="flex flex-col gap-2 text-xs min-w-[200px] max-w-[300px]">
            {[
              { label: t('Psevdonim'), value: userAlias },
              { label: "Email", value: userEmail },
              { label: t('ID_polzovatelya'), value: userId },
            ].map(({ label, value }) => (
    ...
    },
    {
      ...
      cell: (info) => {
        const value = info.getValue();
        return value ? new Date(value as string).toLocaleDateString() : t('Nikogda');
      },
    },
    {
      id: "last_active",
      ...
      header: () => (
        <span className="flex items-center gap-1">
          {t('Poslednyaya_aktivnost')}
          <Popover
            content={t('Eto_novoe_pole_dannye_ne_vosstanavlivayu')}
            trigger="hover"
          >
            <InfoCircleOutlined className="text-gray-400 text-xs cursor-help" />
          </Popover>
    ...
    },
    {
      ...
      header: t('Sbros_byudzheta'),
      size: 130,
      enableSorting: false,
      cell: (info) => {
        const value = info.getValue();
        return value ? new Date(value as string).toLocaleString() : t('Nikogda');
      },
    },
    {
      id: "models",
                      ...
                      <div className="flex flex-wrap gap-1">
                        ...
                        {models.length > 3 && !expandedAccordions[info.row.id] && (
                          <Badge size={"xs"} color="gray" className="cursor-pointer">
                            <Text>
                              +{models.length - 3} {models.length - 3 === 1 ? t('model') : t('modeley')}
                            </Text>
                          </Badge>
                        )}
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div>
            <div>TPM: {key.tpm_limit !== null ? key.tpm_limit : t('Bezlimitno')}</div>
            <div>RPM: {key.rpm_limit !== null ? key.rpm_limit : t('Bezlimitno')}</div>
          </div>
        );
      },
    },
  ...
  const filterOptions: FilterOption[] = [
    ...
    {
      name: "User ID",
      label: t('ID_polzovatelya'),
      isSearchable: false,
    },
    {
      name: "Key Hash",
      label: t('Hesh_klyucha'),
      isSearchable: false,
    },
  ];
            ...
            <div className="inline-flex items-center gap-2">
              {isLoading ? (
                ...
                <span className="inline-flex text-sm text-gray-700">
                  {t('Pokazano')} {rangeLabel} {t('iz')} {totalCount} {t('zapisey')}
                </span>
              )}
              ...
              <AntButton
                ...
                title={t('Zagruzit_dannye')}
              >
                {isButtonLoading ? t('Zagruzka') : t('Zagruzit')}
              </AntButton>
            </div>
            ...
            <div className="inline-flex items-center gap-2">
              {isLoading ? (
                ...
                <span className="text-sm text-gray-700">
                  {t('Stranitsa')} {pageIndex + 1} {t('iz')} {table.getPageCount()}
                </span>
              )}
